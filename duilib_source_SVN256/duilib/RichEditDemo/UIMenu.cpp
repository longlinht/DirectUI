	...
	case 1:
		{
			...
			if (param.lParam != NULL && m_pParentCtrl != NULL)
			{
				...
				::PostMessage(m_hParent, WM_MENU, NULL, (LPARAM)m_pParentCtrl);
			}
		}
		break;
	case 2:
...
void CMenuWnd::Init(CMenuElementUI* pOwner, STRINGorID xml, LPCTSTR pSkinType, POINT point, CControlUI * pParentCtrl/* = NULL*/)
{
	m_BasedPoint = point;
    m_pOwner = pOwner;
    m_pLayout = NULL;
...
void CMenuElementUI::CreateMenuWnd()
{
	...
	m_pWindow->Init(static_cast<CMenuElementUI*>(this), _T(""), _T(""), CPoint(), m_pParent);
}
...
void CMenuElementUI::DrawCheckState(HDC hDC, const RECT& rcItem)
{
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
